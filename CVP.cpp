// Program to solve the Closest Lattice Vector Problem (CVP) for a general crystal lattice  in 3 dimensions
...
const int g_a = 1; // lattice constant
...
{ // finds the closest lattice vector to a_r and stores it in a_n, a_R and returns the distance between a_r and a_R
    ...
    while (1){
        ...
        double dk = 100*g_a; // stores min. distance between r and parallelopiped, initialized to a very large value
...
int main()
{
    cout<<"Solving closest vector problem for 3D bravais lattice\n";
    double r[g_dim]; int n[g_dim]; double R[g_dim];
    cout<<"Enter coordinates of arbitrary point: ";
    cin>>r[0]>>r[1]>>r[2];
    double d = CVP3D(r,n,R);
